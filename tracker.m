classdef tracker < handle
    ...
    properties
        ...
        A1 = [[1,0,0,0]',[0,1,0,0]',[1,0,1,0]',[0,0,0,0]'];  % on table, no vertical velocity
        A2 = [[1,0,0,0]',[0,1,0,0]',[1,0,1,0]',[0,1,0,1]']; % bounce
        A3 = [[1,0,0,0]',[0,1,0,0]',[1,0,1,0]',[0,1,0,1]']; % normal motion
        A4 = [[1,0,0,0]',[0,1,0,0]',[1,0,1,0]',[0,1,0,1]']; % collision
        A5 = [[1,0,0,0]',[0,1,0,0]',[1,0,1,0]',[0,1,0,1]']; % invisible
        ...
        function coords = process_frame(self, frame_number, x_coord, y_coord, appeared)
            ...
            for k = 1 : self.N_HYP
                if i == 1  % make a random vector
                    ...
                    for n = 1 : 4
                        x_current(n) = x_current(n) + 3*sqrt(self.P(k,i-1,n,n))*randn(1);
                    end
                end
                ...
                if i == 1    % initial time - assume falling
                    ...
                    if self.trackstate(k,i-1) == 1  % if already stopped bouncing
                        ...
                        elseif r < (self.p_bounce + self.p_stop + self.p_collision)
                            A = self.A4;
                            Bu = self.Bu4;
                            x_current(3) = -self.loss*x_current(3);
